...
def main():
    ...
    # loading data
    XComp = np.array([xValDummy[0] for xValDummy in Cont1])
    YComp = np.array([xValDummy[1] for xValDummy in Cont1])

    # model run
    object1 = localRegressionWght()
    prediction = [object1.StartTraining(x0InitilizationVal, XComp, YComp) for x0InitilizationVal in XComp]
    object1.plot(XComp, prediction)
    print('END Q2_AB\n')
